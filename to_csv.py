...
pickle_pops_dir = r'D:\Work\smile_desk\simulating_17'
pickle_csv_dir = r'D:\Work\smile_desk\simulating_17_csv'
...
def dump_to_csv_file(df, filename, filesuffix='.csv', 
                     dirname=None, create_newdir=True, avoid_overwrite=True):
    #build path
...
for n in range(nsims):
    for firstday in (3, 7, 10, 15):

        #Filtered
        dfs = load_from_file(pickle_pops_dir+rf"\filteredout_persons_{n}_{firstday}.pik")
        for i,j in np.ndindex(poplists_shape):
            ...
            for l in df.index.levels[0]: #pop indices
                dump_to_csv_file(df.xs(l), foldername+f"_population_{n*npops_per_sim+l}_filteredout_persons_{firstday}", filesuffix='.csv', 
                                dirname=os.path.join(pickle_csv_dir, foldername), create_newdir=True, avoid_overwrite=True)

        #Sampled
        poplists = load_from_file(pickle_pops_dir+f"\sampled_poplists_{n}_{firstday}.pik")
        for i,j, k in np.ndindex(poplists.shape):
            ...
            for l in range(len(poplist)):
                ...
                dump_to_csv_file(dfdays, foldername+f"_population_{n*npops_per_sim+l}_sampled_{method_options[k]}_daysonly_{firstday}", filesuffix='.csv',
                                dirname=os.path.join(pickle_csv_dir, foldername), create_newdir=True, avoid_overwrite=True)
        print(n)
